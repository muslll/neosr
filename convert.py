...
def to_onnx():
    ...
    np.testing.assert_allclose(torch_out.detach().cpu().numpy(),
                               ort_outs[0],
                               rtol=0.01,
                               atol=0.001)
    ...
    if args.optimize:
        ...
        session_opt.graph_optimization_level = onnxruntime.GraphOptimizationLevel.ORT_ENABLE_EXTENDED
    ...
    if args.fp16:
        ...
        print(f'-------- Model successfully converted to half-precision. Saved at: {output_fp16}.')
    ...
    if args.fulloptimization:
        ...
        if args.network in opt_error:
            raise RuntimeError(f'Network [{args.network}] doesnt support full optimization.')
        ...
        print(f'-------- Model successfully optimized. Saved at: {output_fp32_fulloptimized}\n')
