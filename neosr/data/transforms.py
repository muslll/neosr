...
def cutblur(im1, im2, prob=1.0, alpha=0.8):
    """
    ...
    alpha_rand = random.uniform(0.2, alpha)
    cut_ratio = torch.randn(1) * 0.01 + alpha_rand
    ...
    if torch.rand(1) > 0.5:
        ...
        # apply LQ to GT
        im1[..., cy : cy + ch, cx : cx + cw] = im2_aug[..., cy : cy + ch, cx : cx + cw]
    else:
        im2_aug = im1.clone()
