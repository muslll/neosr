...
class ResidualDenseDeformableBlock(nn.Module):
    ...
    def __init__(self, num_feat=64, num_grow_ch=32):
        ...
        self.offset4 = nn.Conv2d(num_feat + 3 * num_grow_ch,
                                     ...
                                     1,
                                     bias=True)
        nn.init.constant_(self.offset_conv2.weight, 0.)
        nn.init.constant_(self.offset_conv2.bias, 0.)
        
        self.conv5 = nn.Conv2d(num_feat + 4 * num_grow_ch, num_feat, 3, 1, 1)

        self.offset_conv5 = nn.Conv2d(num_feat + 4 * num_grow_ch,
                                     2 * 3 * 3,
                                     3,
                                     1,
                                     1,
    ...
    def forward(self, x):
        ...
        x2= deform_conv2d(x, offset=self.offset2(x), weight=self.conv2.weight,
                                          bias=self.conv2.bias,stride=1,padding=1)
        #x3 = self.lrelu(self.conv3(torch.cat((x, x1, x2), 1)))
        x3= deform_conv2d(x, offset=self.offset3(x), weight=self.conv3.weight,
                                          bias=self.conv3.bias,stride=1,padding=1)
        #x4 = self.lrelu(self.conv4(torch.cat((x, x1, x2, x3), 1)))
        x4= deform_conv2d(x, offset=self.offset4(x), weight=self.conv4.weight,
                                          bias=self.conv4.bias,stride=1,padding=1)
        #x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1))
        x5= deform_conv2d(x, offset=self.offset5(x), weight=self.conv5.weight,
